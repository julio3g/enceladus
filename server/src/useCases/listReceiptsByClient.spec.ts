...
describe('List Clients', () => {
  ...
  it('should be able to get balance from list of receipts by client', async () => {
    await receiptsRepository.create({
      description: 'description-1',
      value: 1,
      client_id: 'client-1',
    ...
    const { generalBalanceOfReceipts } = await sut.execute({
      clientId: 'client-1',
    })

    expect(generalBalanceOfReceipts).toEqual(6)
  })
})
